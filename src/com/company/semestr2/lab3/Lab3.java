...
public class Lab3 {
    ...
    public static void main(String[] args) {
        ...
        List<Double> p = calc(x, Lab3::functionP, N, h);
        List<Double> g = calc(x, Lab3::functionG, N, h);
        List<Double> f = calc(x, Lab3::functionF, N, h);
        ...
        List<Double> eps = calcEpsylon(a, b, c, N);
...
//        }
    ...
    private static List<Double> calcEpsylon(List<Double> a, List<Double> b, List<Double> c, int N) {
        List<Double> result = new ArrayList<>();
        int m = N / 2 - 1;
        int n = N - 1;
        ...
        for (int i = m - 1; i >= 0; i--) result.add(null);
    ...
    private static List<Double> calc(List<Double> xList, Function<Double, Double> func, int n, double h) {
        List<Double> result = new ArrayList<>();
        for (double x: xList) {
    ...
    private static List<Double> reverseList(List<Double> list) {
        return IntStream
                .range(0, list.size())
                .map(i -> (list.size() - 1 - i))
                .mapToObj(list::get)
